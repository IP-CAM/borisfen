						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("
								...
								SET 
									...
									is_default = '" . $this->db->escape($product_option_value['is_default']) . "'
							");
							// Dependent Options
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							...
							$this->db->query("
								...
								SET 
									...
									is_default = '" . (($product_option_value['product_option_value_id'] == $this->request->post['default_option_value_id']) ? 1 : 0) . "'
							");
							// Dependent Options
							$product_option_value_id = $this->db->getLastId();
							$dependent_option_value[$product_option_value['option_value_id']] = $product_option_value_id;
...
// 		$this->linkOptionToFilter($product_id);
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
        if($query->num_rows) {
            $liters = $this->getProductLitersName($product_id);
            if($liters) {
                $query->row['name'] .= ' (' . $liters . ')';
            }
        }
		return $query->row;
	}
	...
	public function getProducts($data = array()) {
        ...
        if(!empty($query->rows)) {
            ...
            foreach ($query->rows as $row) {
                $liters = $this->getProductLitersName($row['product_id']);
                if($liters) {
                    $query->rows[$i]['name'] .= ' (' . $liters . ')';
                }
                $i++;
            }
	...
	//die("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_option WHERE option_id = '" . (int)$option_id . "'");
		return $query->row['total'];
	}	
    ...
    public function getProductLitersName($product_id) {
        $sql = "
            Select
                ovd.name as liters_name
            From
                " . DB_PREFIX . "product_option_value pov
            Left Join
                " . DB_PREFIX . "option_value_description ovd
            On
                (ovd.option_value_id = pov.option_value_id)
            Where
                pov.product_id = " . (int)$product_id . "
            And
                pov.option_id = " . (int)OPTION_LITTERS . "
            Order By pov.option_value_id DESC
            Limit 1
        ";
        $liters = $this->db->query($sql);

        if($liters->num_rows) {
            return $liters->row['liters_name'];
        } else {
            return '';
        }
    }
