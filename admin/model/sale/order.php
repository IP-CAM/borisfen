...
class ModelSaleOrder extends Model {
	...
	private function eventEditOrder($order_id, $data){
		if($data['order_status_id'] == $this->config->get('config_complete_status_id')) {
			...
			if ($order_query->num_rows) {
				...
				// Бонусы для розничных покупателей
				if ($order_query->row['customer_id']){
					// Проверить группу пользователя
					$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE customer_id = '" . $order_query->row['customer_id'] . "'");

					// 1. Получить сумму всех оплаченных заказов пользователя
					$sum_totals = $this->db->query("SELECT SUM(total) as sum FROM " . DB_PREFIX . "order
														WHERE customer_id = '" . $order_query->row['customer_id'] . "'
														AND order_status_id = '" . $this->config->get('config_complete_status_id') . "'");
					$sum_totals = $sum_totals->row['sum'];

					//TODO: add transfering user to group
	...
	public function editOrder($order_id, $data) {
		...
		$this->eventEditOrder($order_id, $data);
