...
<div class="container">
    ...
    <h2><?php echo $heading_address; ?></h2>
		...
		<div class="pull-right">
            <a href="<?php echo $cart ?>"class="btn btn-primary"><span><?php echo $text_cart ?></span></a>
            <a class="btn btn-primary" id="continue-button"><span><?php echo $text_continue ?></span></a>
			</div>
        </div>
        <input type="hidden" name="addressSelected" value="0" class="form-control"  />
        ...
        new CBA.Widgets.AddressWidget({
            ...
            onAddressSelect: function(widget) {
                ...
                $.get('<?php echo $shipping_quotes ?>', {}, function(data) {
                    ...
                    if (data.error) {
                        $('#amazon-address-widget').before('<div class="alert alert-warning">' + data.error + '</div>');
                    } else if (data.quotes) {
                        var html = '';
