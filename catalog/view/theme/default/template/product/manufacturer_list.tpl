...
<div class="container">
  ...
  <div class="row"><?php echo $column_left; ?>
    ...
    <div class="col-xs-<?php echo $cols; ?>"><?php echo $content_top; ?>
      ...
      <div class="row manufacturer_list">
        <?php for ($i = 0; $i < count($category['manufacturer']);) { ?>
        <?php $j = $i + ceil(count($category['manufacturer']) / 4); ?>
        <?php for (; $i < $j; $i++) { ?>
        <?php if (isset($category['manufacturer'][$i])) { ?>
        <div class="col-xs-3"><a href="<?php echo $category['manufacturer'][$i]['href']; ?>"><?php echo $category['manufacturer'][$i]['name']; ?></a></div>
        <?php } ?>
        <?php } ?>
        <?php } ?>
      </div>
      <div class="plus-empty-10"></div>
      <?php } ?>
      <?php } ?>
      <?php } else { ?>
