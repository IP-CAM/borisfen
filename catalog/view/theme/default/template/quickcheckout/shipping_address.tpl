...
<div id="shipping_address_wrap" <?php echo (!$data['display'] || $payment_address['shipping']) ? 'class="hide"' : ''; ?>>
  <div class="box box-border">
    <div class="heading"><span><i class="fa fa-globe"></i> <?php echo $data['title']; ?></span></div>
    <div class="box-content">
      ...
      <div id="<?php echo $field['id']; ?>_input" class="box box-border sort-item form-group <?php echo ($field['class'])? $field['class'] : ''; ?>" data-sort="<?php echo $field['sort_order']; ?>">
        <div class="heading"><?php echo $field['title']; ?></div>
        <div class="box-content">
         <div>
          <?php } ?>
          <?php		break; 
          ...
          <div id="<?php echo $field['id']; ?>_input" class="text-input sort-item form-group <?php echo (!$field['display'])? 'hide' : ''; ?> <?php echo ($field['class'])? $field['class'] : ''; ?>" data-sort="<?php echo $field['sort_order']; ?>">
            <label for="shipping_address_<?php echo $field['id']; ?>"> <span class="required <?php echo (!isset($field['require']) ||  !$field['require']) ? 'hide' : ''; ?>">*</span> <span class="text"><?php echo $field['title']; ?></span>  <?php echo (!empty($field['tooltip']))? '<i class="icon-help" rel="tooltip" data-help="'.$field['tooltip'] .'"></i>' : '' ; ?></label>
            <input type="text" class="form-control" name="shipping_address[<?php echo $field['id']; ?>]" data-require="<?php echo (isset($field['require']) && $field['require']) ? 'require' : ''; ?>" data-refresh="<?php echo ($field['refresh']) ? $field['refresh'] : 0; ?>" id="shipping_address_<?php echo $field['id']; ?>" value="<?php echo isset($field['value'])? $field['value'] : ''; ?>" />
          </div>
          <?php } ?>
          <?php } ?>
          <?php } ?>
  ...
  <script type="text/javascript"><!--
    $(document).on('click', 'input[name=\'shipping_address[exists]\']', function() {
     if (this.value == '0') {
    ...
    $('#shipping_address_wrap select[name=\'shipping_address[zone_id]\']').selectpicker('refresh');
  },
