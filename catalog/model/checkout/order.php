	...
	public function addOrder($data) {
		$this->db->query("
			...
			SET 
				...
				payment_country = '" . ($this->config->get('use_countries_and_regions')?$this->db->escape($data['payment_country']):0) . "', 
				payment_country_id = '" . ($this->config->get('use_countries_and_regions')?((int)$data['payment_country_id']):0) . "', 
				payment_zone = '" . ($this->config->get('use_countries_and_regions')?$this->db->escape($data['payment_zone']):0) . "', 
				payment_zone_id = '" . ($this->config->get('use_countries_and_regions')?((int)$data['payment_zone_id']):0) . "', 
				...
				shipping_country = '" . ($this->config->get('use_countries_and_regions')?$this->db->escape($data['shipping_country']):0) . "', 
				shipping_country_id = '" . ($this->config->get('use_countries_and_regions')?((int)$data['shipping_country_id']):0) . "', 
				shipping_zone = '" . ($this->config->get('use_countries_and_regions')?$this->db->escape($data['shipping_zone']):0) . "', 
				shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', 
				shipping_address_format = '" . $this->db->escape($data['shipping_address_format']) . "', 
				shipping_method = '" . ($this->config->get('use_shipping_methods')?$this->db->escape($data['shipping_method']):0) . "', 
				shipping_code = '" . ($this->config->get('use_shipping_methods')?$this->db->escape($data['shipping_code']):0) . "', 
...
// PRODUCTS
			...
			if ($this->config->get('config_alert_mail')) {
				...
				foreach ($order_product_query->rows as $product) {
					$text .= $product['quantity'] . 'x ' . $product['name'] . ' (' . $product['model'] . ') ' . html_entity_decode($this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'], true, false), ENT_NOQUOTES, 'UTF-8') . "\n";
				...
				foreach ($order_voucher_query->rows as $voucher) {
					$text .= '1x ' . $voucher['description'] . ' ' . $this->currency->format($voucher['amount'], $order_info['currency_code'], $order_info['currency_value'], true, false);
				}
			...
			$exports['order_id'] = $order_info['order_id'];
			...
			// Create file
			file_put_contents(DIR_ORDERS . 'order_' . $exports['order_id']  . '.xml', $content);
		}
	}
