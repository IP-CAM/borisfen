...
class ModelPavblogBlog extends Model {		
    ...
    public function getProductsByTags($blog_id) {
        // get blog tags and prepare int
        $sql = "
            Select tags From " . DB_PREFIX . "pavblog_blog Where blog_id = " . (int)$blog_id . "
        ";
        ...
        if($blog->num_rows) {
            ...
            foreach($blog as $blog) {
                $tags[] = 'pd.`tag` like \'%' . trim(utf8_strtolower($blog)) . '%\'';
            }

            $condition = implode(' OR ', $tags);
	...
	public function getNearesBlog() {
		$sql = "
            Select
                ...
                b.blog_id
            From
                " . DB_PREFIX . "pavblog_blog b
            Left Join
                " . DB_PREFIX . "pavblog_blog_description bd
            On
                bd.blog_id = b.blog_id
            Where
                b.date_event > NOW()
            And
                bd.language_id = " . (int)$this->config->get('config_language_id') . "
            Order By b.date_event LIMIT 1
        ";
		$blog = $this->db->query($sql);
		if($blog->num_rows) {
			return $blog->row;
