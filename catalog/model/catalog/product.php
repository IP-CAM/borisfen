...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
        ...
        //текущая сумма товара в корзине
        $sum_in_cart = $this->cart->getSubTotal();
        $quan_in_cart = $this->cart->countProducts();
		$query->row['price'] = ($is_wholesale && ($query->row['whole_sale_price'] > 0)) ? $query->row['whole_sale_price'] : $query->row['price'];
            
                if(($sum_in_cart >= $this->config->get('config_whole_sale_price_price')) && ($query->row['whole_sale_price'] > 0)){
                    $query->row['price'] = $query->row['whole_sale_price'];
                }
                elseif(($quan_in_cart >= $this->config->get('config_whole_sale_price_quantity')) && ($query->row['whole_sale_price'] > 0)){
                    $query->row['price'] = $query->row['whole_sale_price'];
                }
