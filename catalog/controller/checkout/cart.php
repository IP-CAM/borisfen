...
// 				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $this->url->link('checkout/checkout'));
				/* Custom template for popup notification  */
				...
				foreach ($this->data['cart_products'] as &$cart_product) {
					if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
						$cart_product['single_price'] = $cart_product['price'];
						$cart_product['price'] = $this->currency->format($this->tax->calculate($cart_product['price'] * $cart_product['quantity'], $cart_product['tax_class_id'], $this->config->get('config_tax')));
					}
					if ($cart_product['image'] && file_exists(DIR_IMAGE . $cart_product['image'])) {
						$cart_product['image'] = $this->model_tool_image->resize($cart_product['image'], 150, 150);
					} else {
						$cart_product['image'] = $this->model_tool_image->resize('no_image.jpg', 150, 150);
					}
					$cart_product['href'] = $this->url->link('product/product', 'product_id=' . $cart_product['product_id']);
				}
	...
	public function minicart() {
	    if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
	        if(isset($this->request->post['miniCart']) && $this->request->post['miniCart']) {
	            ...
	            if(isset($this->request->post['products']) && !empty($this->request->post['products'])) {
	                ...
	                foreach ($this->request->post['products'] as $product) {
	                    //Make list of request product ids
	                    $request_products[] = $product['product_id'];
	                    // Update cart products
	                    foreach ($cart_products as $key => $c_product) {
	                        if($c_product['product_id'] == $product['product_id']) {
	                            $this->cart->update($key, $product['quantity']);
	                            $this->request->post['products'][$p]['updated'] = true;
	                        }
	                    }
	                    $p++;
	                }
	                // add not updated products to cart
	                unset($product);
	                foreach ($this->request->post['products'] as $product) {
	                    if(!isset($product['updated']) || !$product['updated']) {
	                        $this->cart->add($product['product_id'], $product['quantity'], false);
	                    }
	                }
	                ...
	                foreach ($cart_products as $key => $product) {
	                    if(!in_array($product['product_id'], $request_products)) {
	            ...
	            // Generate new "Dropdown" cart and return it
	            $this->language->load('checkout/cart');
	            unset($this->session->data['shipping_method']);
	            unset($this->session->data['shipping_methods']);
	            unset($this->session->data['payment_method']);
	            ...
	            if($this->cart->countProducts()) {
	                ...
	                $json['total'] = false;
	                $json['empty'] = sprintf($this->language->get('text_items'), 0, $this->currency->format(0)); 
	            }
