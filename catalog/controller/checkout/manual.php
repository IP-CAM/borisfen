		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {	
			...
			if (isset($this->request->post['order_product'])) {
				foreach ($this->request->post['order_product'] as $order_product) {
					...
					if (!empty($order_product['time_cancel'])) {
						$time_cancel = $order_product['time_cancel'];
					} else {
						$time_cancel = '';
					}
					...
					if ($product_info) {	
						...
						$this->cart->add($order_product['product_id'], $order_product['quantity'], $option_data, false, $time_cancel);
					}
				}
			}
			...
			foreach ($products as $product) {
				...
				if ($product['minimum'] > $product_total && !$product['time_cancel']) {
				...
				if ($product['time_cancel']) {
					$cancel_time = sprintf($this->language->get('text_cancel'), $product['time_cancel']);
					$date_cancel = $product['time_cancel'];
				} else {
					$cancel_time = '';
					$date_cancel = '';
				}
				...
				$json['order_product'][] = array(
					...
					'quantity'   => $product['quantity'] . $cancel_time,
...
// 					$json['error']['shipping']['postcode'] = $this->language->get('error_postcode');
					$this->request->post['shipping_postcode'] = '000000';
				}

				if ($this->request->post['shipping_country_id'] == '') {
// 					$json['error']['shipping']['country'] = $this->language->get('error_country');
					$this->request->post['shipping_country_id'] = false;
				}

				if (!isset($this->request->post['shipping_zone_id']) || $this->request->post['shipping_zone_id'] == '') {
					$this->request->post['shipping_zone_id'] = false;
...
// 				$json['error']['payment']['country'] = $this->language->get('error_country');
				$this->request->post['payment_country_id'] = 1;
			}
