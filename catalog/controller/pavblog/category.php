...
class ControllerPavblogCategory extends Controller {
    ...
    public function index() {
        ...
        if (isset($this->request->get['limit'])) {
            ...
            $limit = (int)$this->mparams->get('cat_limit_leading_blog')
                     + (int)$this->mparams->get('cat_limit_secondary_blog');
        }
        ...
        if ($category_info) {
            ...
            $limageType = isset($type[ $this->mparams->get('cat_leading_image_type') ]) ?
                $type[ $this->mparams->get('cat_leading_image_type') ] : 'thumb_xsmall';
            $simageType = isset($type[ $this->mparams->get('cat_secondary_image_type') ]) ?
                $type[ $this->mparams->get('cat_secondary_image_type') ] : 'thumb_xsmall';
            ...
            foreach ($blogs as $key => $blog) {
                ...
                $blogs[ $key ]['author'] = isset($users[ $blog['user_id'] ]) ? $users[ $blog['user_id'] ] :
                    $this->language->get('text_none_author');
                $blogs[ $key ]['category_link'] = $this->url->link('pavblog/category', "id="
                                                                                       . $blog['category_id']);
            ...
            // Get products related by tags
            $tags = $this->getModel()->getCategoryRecordsTags($category_id);
            $this->load->model('catalog/product');
            $products_ids = $this->model_catalog_product->getProductsByTags($tags);

            $products = array();
            if(!empty($products_ids)) {
                foreach ($products_ids as $product) {
                    // Product category
                    $product_category = $this->model_catalog_product->getDeepestCategory($product['product_id']);
                    ...
                    $products[] = array(
                        ...
                        'href'        => $this->url->link('product/product', 'path=' . $this->model_catalog_product->getFullPath($product['product_id']) . '&product_id=' . $product['product_id']),
                    );
                }
            }
            // Pass related products to view
            ...
            $pagination->url = $this->url->link('pavblog/category', 'id=' . $this->request->get['id'] . $url
                                                                    . '&page={page}');
    ...
    public function rss() {
        ...
        $output .= '<title><![CDATA[' . $category_info['title'] . " - " . $this->config->get('config_name')
                   . ']]></title>';
        ...
        foreach ($blogs as $blog) {
            ...
            if ($blog['image']) {
                ...
                $description = '<a href="' . $link . '"><img class="rss_blog_image" src="' . $image . '"/></a>'
