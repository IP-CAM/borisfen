...
class ControllerModuleFaq extends Controller {
	...
	protected function index() {
		...
		if(($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()){
            ...
            $last_faq_id = $this->model_fido_faq->addFaq($this->request->post);
            ...
            $mail->setText(
                strip_tags(
                    html_entity_decode(
                        ...
                        $this->language->get('text_approve') . ": " . HTTP_SERVER . 'admin/index.php?route=module/faq/update&faq_id=' . $last_faq_id . "\n",
                        ENT_QUOTES,
                        'UTF-8'
                    )
                )
	...
	private function validate(){
		...
		if ((strlen(utf8_decode($this->request->post['title'])) < 5) || (strlen(utf8_decode($this->request->post['title'])) > 600)) {
