  ...
  public function __construct($registry) {
    ...
    if (isset($this->request->get['path'])) {
      ...
      if ($this->products_total && $this->settings['show_price']) {
        ...
        if ($min_max_price) {
          $this->min_price = (int)$min_max_price['min'];
      	  $this->max_price = (int)$min_max_price['max'];
        }
      }
    }
  }
...
//     	$this->language->get('entry_in_stock')
  		$results = array(
			array(
				'value_id' => '1',
				'name' => $this->language->get('entry_in_stock'),
				'option_id' => 's',
			),
			array(
				'value_id' => '2',
				'name' => $this->language->get('entry_out_of_stock'),
				'option_id' => 's',
			),
			array(
				'value_id' => '3',
				'name' => $this->language->get('entry_preorder'),
				'option_id' => 's',
			),
		);
    ...
    // Filter by labels
    $labels = $this->model_catalog_filter->getPromoTags();
    $options[] = array(
    	'option_id'   => 'l',
    	'name'        => $this->language->get('text_special_labels'),
    	'description' => false,
    	'type'        => $this->settings['stock_status_type'],
    	'values'      => $labels
    );    
    ...
    if ($this->settings['manufacturer']) {
  		$results = $this->model_catalog_filter->getManufacturersByCategoryId($this->category_id, $this->filter_id);
      if ($results) {
        $options[] = array(
          'option_id'   => 'm',
	...
	$results = $this->model_catalog_filter->getOptionsByCategoryId($this->category_id, $this->filter_id);
	...
	if ($options) {
        ...
        if ($option['type'] != 'group') {
          if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'radiolabel'|| $option['type'] == 'radiocolor' || $option['type'] == 'custom_text') {
            $values[] = array(
          ...
          for ($i = 0; $i < count($option['values']); $i = $i + $option['group_count']) {
            ...
            if ($group) {
              $values[] = array(
                ...
                'name'     => (count($group) > 1 ? $group[0]['name'] . ' - ' . $group[count($group)-1]['name'] : $group[0]['name']) . '',
                'params'   => $params,
                'selected' => $selected
              );
  ...
  private function params($option_id, $value_id, $type = 'checkbox') {
    ...
    if ($this->params) {
      ...
      if ($type == 'checkbox' || $type == 'checkboxcolor' || $type == 'checkboxlabel') { # Checkbox type
      ...
      } elseif ($type == 'select' || $type == 'slide' || $type == 'radio' || $type == 'radiocolor' || $type == 'radiolabel') { # Selectbox type
        if (isset($key)) { # если эта опция уже есть в запросе, то мы не добавляем её
          if (in_array($value_id, $values[$key])) { # если это значение уже есть в запросе
            $out = str_replace((array_search($parts[$key], $parts) ? $this->sep_par . $parts[$key] : $parts[$key] . $this->sep_par), '', $this->params);
          } else { # если значения нет в запросе, то добавляем его к значениям этой опции
      ...
      # если в запросе вообще нет переменной filter
          ...
          foreach ($category_options[$option_id]['values'] as $value) { # Мы не можем получить доступ к значению по ключу, поэтому перебираем все
            if (in_array($value['value_id'], $values_id)) { # Если есть в GET такое значение - бросаем в выбранные
              ...
              if (count($this->options_get) > 1 || count($values_id) > 1) {
                ...
                if ($type == 'select' || $type == 'radio' || $type == 'radiocolor' || $type == 'radiolabel') {
  ...
  public function callback() {
    ...
    $json['disabled_options'] = array();
    $json['values'] = array();

    if (isset($this->request->get['path'])) {
      foreach ($this->options() as $option) {
      	$positive_values_quantity = 0;
...
//             	if(substr_count($value['params'], 'm')) {
            		$data = array(
            			'filter_category_id' => $this->category_id,
...
//             	}
              ...
              if ($this_option && ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'radiocolor' || $option['type'] == 'radiolabel' || $option['type'] == 'custom_text')) {
...
//                   	}
	      ...
	      if(($count > 0 && !substr_count($value['value_id'], 'cancel')) || empty($option['values'])) {
	        $option_disabled = false;
	        $positive_values_quantity++;
	      }
        }
	    if($option_disabled == true || $positive_values_quantity <= 1) {
	      	$json['disabled_options'][] = array('option_id' => $option['option_id']);
	    } 
      }
    }
    ...
    if ($params) {
      ...
      foreach (explode($this->sep_par, $params) as $option_value) {
        ...
        if (isset($option[1])) {
          ...
          } elseif ($option[0] == 'm' || $option[0] == 's' || $option[0] == 'l') {
            $values = array();
