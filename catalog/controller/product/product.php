...
class ControllerProductProduct extends Controller
{
    ...
    public function index()
    {
        ...
        $this->document->addScript(SCRIPT_FOLDER . 'option-price-live.js');

        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-gallery.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-helper.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-gallery-fullscreen.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-gallery-indicator.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-gallery-video.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-gallery-vimeo.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'blueimp/js/blueimp-gallery-youtube.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'dependent-options.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'jquery/jquery.jcarousel.min.js', true);
        $this->document->addScript(SCRIPT_FOLDER . 'share42.js', true);

        $this->document->addStyle(SCRIPT_FOLDER . 'blueimp/css/blueimp-gallery-indicator.css');
        $this->document->addStyle(SCRIPT_FOLDER . 'blueimp/css/blueimp-gallery-video.css');
        $this->document->addStyle(SCRIPT_FOLDER . 'blueimp/css/blueimp-gallery.css');

        $this->data['breadcrumbs'] = array();

        // Make here check for selected highslide in admin panel @todo: add radio button to select highslide or zoom in admin panel
        if ($this->config->get('config_use_highslide')) {
// 			$this->document->addScript(SCRIPT_FOLDER . 'highslide/highslide-full.js', true);
            $this->document->addFooterStyle(STYLE_FOLDER . 'highslide.css');
        }
		...
		if($product_info['special_end'] && $product_info['special_end'] >= date('Y-m-d H:i:s') && $product_info['special_start'] <= date('Y-m-d H:i:s')) {
            ...
            //var_dump($product_info['special_end']);die();
			$finish = $time->getTimestamp() - time();
			$this->data['special_end'] = $finish;
		}
        ...
        if ($product_info) {
            ...
            if ($product_info['custom_title']) {
                if (isset($category_info) && $category_info['name']) {
                    $this->document->setTitle($category_info['name'] . ' : ' . $product_info['custom_title']);
                } else {
                    $this->document->setTitle($product_info['custom_title']);
                }
            } else {
                if (isset($category_info) && $category_info['name']) {
                    $this->document->setTitle($category_info['name'] . ' : ' . $product_info['name']);
                } else {
            ...
            $this->data['comments_on_product'] = $this->language->get('comments_on_product');
            $this->data['add_a_comment'] = $this->language->get('add_a_comment');
            ...
            if ($product_info['image'] && file_exists(DIR_IMAGE . $product_info['image'])) {
                if (!$this->config->get('config_use_highslide')) {
                    $this->data['popup'] = $this->model_tool_image->scale($product_info['image'], $this->config->get('config_image_popup_width'), 'x');
                } else {
                    $this->data['popup'] = '/image/' . $product_info['image'];
                }
            } else {
                $this->data['popup'] = $this->model_tool_image->scale('no_image.jpg', $this->config->get('config_image_popup_width'), 'x');
            }

            if ($product_info['image'] && file_exists(DIR_IMAGE . $product_info['image'])) {
                $this->data['thumb'] = $this->model_tool_image->scale($product_info['image'], $this->config->get('config_image_thumb_width'), 'x');
            } else {
                $this->data['thumb'] = $this->model_tool_image->scale('no_image.jpg', $this->config->get('config_image_thumb_width'), 'x');
            }

            if ($product_info['image'] && file_exists(DIR_IMAGE . $product_info['image'])) {
                $this->data['image_name'] = $product_info['image'];
            } else {
                $this->data['image_name'] = 'no_image.jpg';
            }
            ...
            foreach ($results as $result) {
                if (!$this->config->get('config_use_highslide')) {
                    $this->data['images'][] = array(
                        'popup' => $this->model_tool_image->resize((($result['image'] && file_exists(DIR_IMAGE . $result['image'])) ? $result['image'] : 'no_image.jpg'), $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),
                        'thumb' => $this->model_tool_image->resize((($result['image'] && file_exists(DIR_IMAGE . $result['image'])) ? $result['image'] : 'no_image.jpg'), $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height')),
                        'name' => ($result['image'] && file_exists(DIR_IMAGE . $result['image'])) ? $result['image'] : 'no_image.jpg',
                    );
                } else {
                    $this->data['images'][] = array(
            ...
            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                $this->data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), '', '', true, false);
            } else {
                $this->data['price'] = false;
            }
            //die($this->data['price']);
            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                $this->data['seo_price'] = $this->currency->format(
                ...
                $this->data['seo_price'] = false;
            }

            if ((float)$product_info['special']) {
                $this->data['special'] = $this->currency->format($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), '', '', true, false);
            } else {
                $this->data['special'] = false;
            }
            if ((float)$product_info['special']) {
                $this->data['seo_special'] = $this->currency->format(
            ...
            foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
                ...
                if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'radiocolor' || $option['type'] == 'radiolabel' || $option['type'] == 'checkbox' || $option['type'] == 'checkboxcolor' || $option['type'] == 'checkboxlabel' || $option['type'] == 'image' || $option['type'] == 'custom_text') {
...
//						if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                        ...
                        $option_value_data[] = array(
                            ...
                            'image' => $this->model_tool_image->resize($option_value['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height')),
                            'bg_image' => $this->model_tool_image->resize($option_value['image'], 26, 26),
                            'image_thumb' => $this->model_tool_image->resize($option_value['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height')),
                            'image_popup' => $this->model_tool_image->resize($option_value['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')),

                            'product_option_image_thumb' => ($option_value['product_option_image'] != 'no_image.jpg') ? $this->model_tool_image->resize($option_value['product_option_image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height')) : false,
                            'product_option_image_popup' => ($option_value['product_option_image'] != 'no_image.jpg') ? $this->model_tool_image->resize($option_value['product_option_image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')) : false,
                            'disabled' => ($option_value['subtract'] && ($option_value['quantity'] < 1) ? true : false),
                            'price' => $price,
...
//						}
            ...
            $this->data['whole_sale_price'] = $this->currency->format($this->tax->calculate($product_info['whole_sale_price'], $product_info['tax_class_id'], $this->config->get('config_tax')), '', '', true, false);

            /* Stock statuses */

            if ($product_info['quantity'] < 1) {
                $this->data['out_of_stock'] = $product_info['stock_status'];
            } else {
                $this->data['out_of_stock'] = false;
            }

            if ($product_info['quantity'] <= $this->config->get('left_a_bit_quantity') && $product_info['quantity'] > 0) {
                $this->data['left_of_stock'] = $this->model_catalog_product->getStockStatus(2);
            } else {
                $this->data['left_of_stock'] = false;
            }

            if (!$this->data['out_of_stock'] && !$this->data['left_of_stock']) {
                $this->data['in_stock'] = $this->model_catalog_product->getStockStatus(1);
            } else {
            ...
            $this->data['short_description'] = strip_tags(html_entity_decode($product_info['short_description'], ENT_QUOTES, 'UTF-8'), '<br><b><i><strong>');
            # start filter
...
// 			$filter_options = $this->model_catalog_filter->getOptionsByProductsId(array($this->request->get['product_id']));
            ...
            if ($this->config->get('config_additional_products_list')) {
                ...
                foreach ($results as $result) {
                    ...
                    $this->data['products'][] = array(
                        ...
                        /*code start*/
                        'promo_tag_top_right' => $promo_tag_product_top_right,
                        'promo_tag_top_left' => $promo_tag_product_top_left,
                        'promo_tag_bottom_left' => $promo_tag_product_bottom_left,
                        'promo_tag_bottom_right' => $promo_tag_product_bottom_right,
                        /*code end*/
                        'special' => $special,
            ...
            if (!$this->config->get('disable_product_series')) {
                $this->data['series_heading'] = $this->language->get('series_heading');
                $series = $this->model_catalog_product->getProductsInSameSeries($this->request->get['product_id']);
                $s_index = 0;
                if (is_array($series) && !empty($series)) {
                    foreach ($series as $sery) {
                        $this->data['series'][$s_index]['name'] = $sery['name'];
                        foreach ($sery['products'] as $single) {
                            if ($single['image'] && file_exists(DIR_IMAGE . $single['image'])) {
                                $image = $this->model_tool_image->resize($single['image'], $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
                            } else {
                                $image = $this->model_tool_image->resize('no_image.jpg', $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
                            }

                            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                                $price = $this->currency->format($this->tax->calculate($single['price'], $single['tax_class_id'], $this->config->get('config_tax')));
                            } else {
                                $price = false;
                            }
                            /*code start*/
                            $promo_top_right = $this->model_catalog_product->getPromo($single['product_id'], $single['promo_top_right']);
                            ...
                            $promo_top_left = $this->model_catalog_product->getPromo($single['product_id'], $single['promo_top_left']);
                            if (!empty($promo_top_left['promo_text']) && $promo_on) {
                                $promo_tag_product_top_left = '<div class="promotags" style="width:70px;height:70px;top:0px;left:0px;background: url(\'' . 'image/' . $promo_top_left['image'] . '\') no-repeat;background-position:top left"></div>';
                            } else {
                                $promo_tag_product_top_left = '';
                            ...
                            $promo_bottom_left = $this->model_catalog_product->getPromo($single['product_id'], $single['promo_bottom_left']);
                            if (!empty($promo_bottom_left['promo_text']) && $promo_on) {
                                $promo_tag_product_bottom_left = '<div class="promotags" style="width:70px;height:70px;bottom:0px;left:0px;background: url(\'' . 'image/' . $promo_bottom_left['image'] . '\') no-repeat;background-position:bottom left"></div>';
                            } else {
                                $promo_tag_product_bottom_left = '';
                            ...
                            $promo_bottom_right = $this->model_catalog_product->getPromo($single['product_id'], $single['promo_bottom_right']);
                            if (!empty($promo_bottom_right['promo_text']) && $promo_on) {
                                $promo_tag_product_bottom_right = '<div class="promotags" style="width:70px;height:70px;bottom:0px;right:0px;background: url(\'' . 'image/' . $promo_bottom_right['image'] . '\') no-repeat;background-position:bottom right"></div>';
                            } else {
                                $promo_tag_product_bottom_right = '';
                            ...
                            if ((float)$single['special']) {
                                $special = $this->currency->format($this->tax->calculate($single['special'], $single['tax_class_id'], $this->config->get('config_tax')));
                            } else {
                                $special = false;
                            }

                            if ($this->config->get('config_review_status')) {
                                $rating = (int)$single['rating'];
                            } else {
                                $rating = false;
                            }
                            ...
                            if ($single['quantity'] < 1) {
                                $out_of_stock = $single['stock_status'];
                            } else {
                                $out_of_stock = false;
                            }

                            if ($single['quantity'] <= $this->config->get('left_a_bit_quantity') && $single['quantity'] > 0) {
                                $left_of_stock = $this->model_catalog_product->getStockStatus(2);
                            } else {
                                $left_of_stock = false;
                            }
                            ...
                            $product_options = $this->model_catalog_product->getProductOptions($single['product_id']);

                            //print_r($product_options);

                            $litters = array();
                            foreach($product_options as $option){
                                if($option['option_id'] == OPTION_LITTERS){ // 40
                                    foreach($option['option_value'] as $opt_val){
                                        if($opt_val['is_default']){
                                            $litters = $opt_val;
                                        }
                                    }
                                }
                            ...
                            if(count($litters)){
                                $this->data['series'][$s_index]['products'][] = array(
                                    ...
                                    'left_of_stock' => $left_of_stock,
                                    'in_stock' => $in_stock,
                                    'in_cart' => in_array($single['product_id'], $cart_ids),
                                    'description' => strip_tags(html_entity_decode($single['description'], ENT_QUOTES, 'UTF-8')),
                                    'short_description' => strip_tags(html_entity_decode($single['short_description'], ENT_QUOTES, 'UTF-8'), '<b><i><strong>'),
                                    'promo_tag_top_right' => $promo_tag_product_top_right,
                                    'promo_tag_top_left' => $promo_tag_product_top_left,
                                    'promo_tag_bottom_left' => $promo_tag_product_bottom_left,
                                    'promo_tag_bottom_right' => $promo_tag_product_bottom_right,
                                    'special' => $special,
                                    'special_end' => $single['special_end'],
                                    'rating' => $rating,
                                    'description' => utf8_substr(strip_tags(html_entity_decode($single['description'], ENT_QUOTES, 'UTF-8')), 0, 100) . '...',
                                    'reviews' => sprintf($this->language->get('text_reviews'), (int)$single['reviews']),
                                    'href' => $this->url->link('product/product', 'product_id=' . $single['product_id']),
                                    'litters' => $litters,
                                    'is_current_product' => $this->request->get['product_id'] == $single['product_id'] ? true : false
                                );
                            }
                        ...
                        // sort
                        usort($this->data['series'][$s_index]['products'], 'cmp');
            ...
            $this->data['product_option_tpl'] = $this->render();

            // Product Review
            $this->template = $this->config->get('config_template') . '/template/product/product_review.tpl';
            $this->data['product_review_tpl'] = $this->render();


            // Product Series
            $this->template = $this->config->get('config_template') . '/template/product/product_series.tpl';
            $this->data['product_series_tpl'] = $this->render();

            // Product Related
            $this->template = $this->config->get('config_template') . '/template/product/product_related.tpl';
            $this->data['product_related_tpl'] = $this->render();
    ...
    public function voteAdd()
    {
        if (
            ...
            $key = $this->getVoteKey($this->request->post['product_id']);
            if (!$key) {
                $this->load->model('catalog/review');
                $key = $this->createVoteKey($this->request->post['product_id']);
                $this->model_catalog_review->addVote($this->request->post['rating'], $this->request->post['product_id'], $key);
                exit(json_encode(array('key' => $key, 'product_id' => $this->request->post['product_id'])));
            } else {
                $this->voteReplace();
            }
        } else {
    ...
    public function voteReplace()
    {
        if (
            ...
            $this->load->model('catalog/review');
            $key = $this->getVoteKey($this->request->post['product_id']);
            $this->model_catalog_review->updateVote($this->request->post['rating'], $this->request->post['product_id'], $key);
    ...
    private function getVoteKey($product_id)
    {
        if (isset($_COOKIE['vote_' . $product_id])) {
            return $_COOKIE['vote_' . $product_id];
        } else {
            return false;
        }
    }

    private function createVoteKey($product_id)
    {
        if (isset($_COOKIE['PHPSESSID'])) {
            $key = md5($_COOKIE['PHPSESSID'] . rand(0, 65535) . $product_id);
        } else {
            $key = md5('without-session 0o' . rand(0, 65535) . $product_id);
        }
...
function cmp($a, $b){
    return strcmp($a['litters']['name'], $b['litters']['name']);
}
