...
class ControllerInformationContact extends Controller {
    ...
    public function year() {
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            $this->load->language('information/callme');
            if(isset($this->request->post) && !empty($this->request->post)) {
                if($this->validateYear($this->request->post)) {
                    $mail = new Mail();
                    $mail->protocol = $this->config->get('config_mail_protocol');
                    $mail->parameter = $this->config->get('config_mail_parameter');
                    $mail->hostname = $this->config->get('config_smtp_host');
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/year.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/information/year.tpl';
            } else {
                $this->template = 'default/template/information/year.tpl';
            }
            $this->response->setOutput($this->render());
        }
    }

    private function validateYear($post) {
        $this->error = array();
        if(!isset($post['phone']) || strlen($post['phone']) < 6) {
            $this->error = array('phone');
        }
