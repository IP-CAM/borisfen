...
if(PAGE_CACHING) {
	...
	if(!empty($_SERVER['REQUEST_METHOD'])
        ...
        && empty($_SERVER['HTTPS'])
        && (!isset($_GET['filter_params']) || !$_GET['filter_params'])
        && ($_SERVER['REQUEST_URI'] != '/faq')
        && ($_SERVER['REQUEST_URI'] != '/vopros-otvet')
    ) {
...
function addBar($content, $requestKey) {
	...
	<body\$1><div id="pageCacheBar" style="">
		<p style="margin: 0; color:#000;"><input type="button" onclick="clearcache()" value="Очистить кеш для этой страницы" /> <b>Размер без кеширования: {without}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Размер кешированной копии: {with}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b><b>Клёво, правда? :)</b></p>
	</div>
